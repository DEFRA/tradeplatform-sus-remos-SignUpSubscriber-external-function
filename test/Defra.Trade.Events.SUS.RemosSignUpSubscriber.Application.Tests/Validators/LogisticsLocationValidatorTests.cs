...
public sealed class LogisticsLocationValidatorTests : ValidatorTestBase<LogisticsLocation>
{
    private readonly IValidator<Address?> _addressValidator;
    private readonly LogisticsLocationValidator _sut;

    public LogisticsLocationValidatorTests()
    {
        _addressValidator = A.Fake<IValidator<Address?>>(opt => opt.Strict());
        _sut = new LogisticsLocationValidator(_addressValidator);
    }
